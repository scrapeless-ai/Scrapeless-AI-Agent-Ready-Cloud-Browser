    ...
    pub fn format(&self) -> String {
        match self {
            ...
            ParseError::MissingArguments { context, usage } => {
                format!(
                    "Missing arguments for: {}\nUsage: scrapeless-scraping-browser {}",
                    context, usage
                )
            }
            ParseError::InvalidValue { message, usage } => {
                format!("{}\nUsage: scrapeless-scraping-browser {}", message, usage)
            }
...
pub fn parse_command(args: &[String], flags: &Flags) -> Result<Value, ParseError> {
    ...
    match cmd {
        "config" => {
            if args.len() < 2 {
                return Err(ParseError::MissingArguments {
                    ...
                    usage: "config <set|get|list|remove> [args]",
                });
            }
            ...
            match subcommand.as_str() {
                "set" => {
                    ...
                    // Map 'key' to 'apiKey' for internal storage
                    let config_key = if key == "key" { "apiKey" } else { key };
                ...
                "remove" => {
                    if args.len() < 3 {
                        return Err(ParseError::MissingArguments {
                            context: "config remove".to_string(),
                ...
                _ => Err(ParseError::UnknownSubcommand {
                    ...
                    valid_options: &["set", "get", "list", "remove"],
                })
            }
        }
        ...
        "sessions" => Ok(json!({ "id": id, "action": "scrapeless_sessions" })),
        
        "new-session" => {
            // Create a new Scrapeless session with optional parameters
            let mut cmd = json!({ "id": id, "action": "scrapeless_create" });
            ...
            while i < rest.len() {
                match rest[i] {
                    ...
                    "--ttl" => {
                        if let Some(ttl) = rest.get(i + 1) {
                            if let Ok(t) = ttl.parse::<u32>() {
                                cmd["sessionTtl"] = json!(t);
                            }
                            i += 1;
                        }
                    }
                    "--recording" => {
                        if let Some(recording) = rest.get(i + 1) {
                            let rec_bool = recording.to_lowercase() == "true";
                            cmd["sessionRecording"] = json!(rec_bool);
                            i += 1;
                        }
                    }
                    "--proxy-country" => {
                        if let Some(country) = rest.get(i + 1) {
                            cmd["proxyCountry"] = json!(country);
                            i += 1;
                        }
                    }
                    "--proxy-state" => {
                        if let Some(state) = rest.get(i + 1) {
                            cmd["proxyState"] = json!(state);
                            i += 1;
                        }
                    }
                    "--proxy-city" => {
                        if let Some(city) = rest.get(i + 1) {
                            cmd["proxyCity"] = json!(city);
                            i += 1;
                        }
                    }
                    "--user-agent" => {
                        if let Some(ua) = rest.get(i + 1) {
                    ...
                    "--platform" => {
                        if let Some(platform) = rest.get(i + 1) {
                            cmd["platform"] = json!(platform);
                            i += 1;
                        }
                    }
                    "--screen-width" => {
                        if let Some(width) = rest.get(i + 1) {
                            if let Ok(w) = width.parse::<u32>() {
                                cmd["screenWidth"] = json!(w);
                            }
                            i += 1;
                        }
                    }
                    "--screen-height" => {
                        if let Some(height) = rest.get(i + 1) {
                            if let Ok(h) = height.parse::<u32>() {
                                cmd["screenHeight"] = json!(h);
                            }
                            i += 1;
                        }
                    }
                    "--timezone" => {
                        if let Some(tz) = rest.get(i + 1) {
                            cmd["timezone"] = json!(tz);
                            i += 1;
                        }
                    }
                    "--languages" => {
                        if let Some(langs) = rest.get(i + 1) {
                            let lang_list: Vec<&str> = langs.split(',').collect();
                            cmd["languages"] = json!(lang_list);
                            i += 1;
                        }
                    }
                    _ => {}
        ...
        "stop" => {
            let task_id = rest.first().ok_or_else(|| ParseError::MissingArguments {
                context: "stop".to_string(),
                usage: "stop <taskId>",
            })?;
            Ok(json!({ "id": id, "action": "scrapeless_stop", "taskId": task_id }))
        }
        
        "stop-all" => Ok(json!({ "id": id, "action": "scrapeless_stop_all" })),
        
        "live" => {
            // If taskId is provided, get live URL for that session
            // Otherwise, get live URL for current session
            let mut cmd = json!({ "id": id, "action": "scrapeless_live" });
            if let Some(task_id) = rest.first() {
                cmd["taskId"] = json!(task_id);
            }
            Ok(cmd)
        },
            ...
            match sub {
                Some("save") => {
                    let name = rest.get(1).ok_or_else(|| ParseError::MissingArguments {
                        ...
                        usage: "scrapeless-scraping-browser auth save <name> --url <url> --username <user> --password <pass>",
                    })?;
                    ...
                    let url_val = url.ok_or_else(|| ParseError::MissingArguments {
                        context: "auth save".to_string(),
                        usage: "scrapeless-scraping-browser auth save <name> --url <url> --username <user> --password <pass> [--password-stdin]",
                    })?;
                ...
                Some("login") => {
                    let name = rest.get(1).ok_or_else(|| ParseError::MissingArguments {
                        ...
                        usage: "scrapeless-scraping-browser auth login <name>",
                    })?;
                ...
                Some("delete") | Some("remove") => {
                    let name = rest.get(1).ok_or_else(|| ParseError::MissingArguments {
                        ...
                        usage: "scrapeless-scraping-browser auth delete <name>",
                    })?;
                ...
                Some("show") => {
                    let name = rest.get(1).ok_or_else(|| ParseError::MissingArguments {
                        ...
                        usage: "scrapeless-scraping-browser auth show <name>",
                    })?;
        ...
        "confirm" => {
            let cid = rest.first().ok_or_else(|| ParseError::MissingArguments {
                ...
                usage: "scrapeless-scraping-browser confirm <confirmation-id>",
            })?;
        ...
        "deny" => {
            let cid = rest.first().ok_or_else(|| ParseError::MissingArguments {
                ...
                usage: "scrapeless-scraping-browser deny <confirmation-id>",
            })?;
