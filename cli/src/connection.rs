...
pub fn get_socket_dir() -> PathBuf {
    ...
    if let Ok(dir) = env::var("SCRAPELESS_BROWSER_SOCKET_DIR") {
        if !dir.is_empty() {
    ...
    if let Some(home) = dirs::home_dir() {
        return home.join(".scrapeless-scraping-browser");
    }
...
fn apply_daemon_env(cmd: &mut Command, session: &str, opts: &DaemonOptions) {
    cmd.env("SCRAPELESS_BROWSER_DAEMON", "1")
        .env("SCRAPELESS_BROWSER_SESSION", session);

    if opts.headed {
        cmd.env("SCRAPELESS_BROWSER_HEADED", "1");
    }
    if opts.debug {
        cmd.env("SCRAPELESS_BROWSER_DEBUG", "1");
    }
    if let Some(path) = opts.executable_path {
        cmd.env("SCRAPELESS_BROWSER_EXECUTABLE_PATH", path);
    }
    if !opts.extensions.is_empty() {
        cmd.env("SCRAPELESS_BROWSER_EXTENSIONS", opts.extensions.join(","));
    }
    if let Some(a) = opts.args {
        cmd.env("SCRAPELESS_BROWSER_ARGS", a);
    }
    if let Some(ua) = opts.user_agent {
        cmd.env("SCRAPELESS_BROWSER_USER_AGENT", ua);
    }
    if let Some(p) = opts.proxy {
        cmd.env("SCRAPELESS_BROWSER_PROXY", p);
    }
    if let Some(pb) = opts.proxy_bypass {
        cmd.env("SCRAPELESS_BROWSER_PROXY_BYPASS", pb);
    }
    if opts.ignore_https_errors {
        cmd.env("SCRAPELESS_BROWSER_IGNORE_HTTPS_ERRORS", "1");
    }
    if opts.allow_file_access {
        cmd.env("SCRAPELESS_BROWSER_ALLOW_FILE_ACCESS", "1");
    }
    if let Some(prof) = opts.profile {
        cmd.env("SCRAPELESS_BROWSER_PROFILE", prof);
    }
    if let Some(st) = opts.state {
        cmd.env("SCRAPELESS_BROWSER_STATE", st);
    }
    if let Some(p) = opts.provider {
        cmd.env("SCRAPELESS_BROWSER_PROVIDER", p);
    }
    if let Some(d) = opts.device {
        cmd.env("SCRAPELESS_BROWSER_IOS_DEVICE", d);
    }
    if let Some(sn) = opts.session_name {
        cmd.env("SCRAPELESS_BROWSER_SESSION_NAME", sn);
    }
    if let Some(dp) = opts.download_path {
        cmd.env("SCRAPELESS_BROWSER_DOWNLOAD_PATH", dp);
    }
    if let Some(ad) = opts.allowed_domains {
        cmd.env("SCRAPELESS_BROWSER_ALLOWED_DOMAINS", ad.join(","));
    }
    if let Some(ap) = opts.action_policy {
        cmd.env("SCRAPELESS_BROWSER_ACTION_POLICY", ap);
    }
    if let Some(ca) = opts.confirm_actions {
        cmd.env("SCRAPELESS_BROWSER_CONFIRM_ACTIONS", ca);
    }
    if let Some(engine) = opts.engine {
        cmd.env("SCRAPELESS_BROWSER_ENGINE", engine);
    }
}
...
pub fn ensure_daemon(session: &str, opts: &DaemonOptions) -> Result<DaemonResult, String> {
        ...
        if path_len > 103 {
            return Err(format!(
                "Session name '{}' is too long. Socket path would be {} bytes (max 103).\n\
                 Use a shorter session name or set SCRAPELESS_BROWSER_SOCKET_DIR to a shorter path.",
                session, path_len
            ));
        }
    }
    ...
    if opts.native {
        ...
        if let Ok(home) = env::var("SCRAPELESS_BROWSER_HOME") {
        ...
        let daemon_path = daemon_paths
            .iter()
            .find(|p| p.exists())
            .ok_or("Daemon not found. Set SCRAPELESS_BROWSER_HOME environment variable or run from project directory.")?;
...
mod tests {
    ...
    fn test_get_socket_dir_explicit_override() {
        let _guard = EnvGuard::new(&["SCRAPELESS_BROWSER_SOCKET_DIR", "XDG_RUNTIME_DIR"]);

        _guard.set("SCRAPELESS_BROWSER_SOCKET_DIR", "/custom/socket/path");
    ...
    fn test_get_socket_dir_ignores_empty_socket_dir() {
        ...
        _guard.set("SCRAPELESS_BROWSER_SOCKET_DIR", "");
        _guard.remove("XDG_RUNTIME_DIR");

        assert!(get_socket_dir()
            .to_string_lossy()
            .ends_with(".scrapeless-scraping-browser"));
    }
    ...
    fn test_get_socket_dir_xdg_runtime() {
        ...
        _guard.remove("SCRAPELESS_BROWSER_SOCKET_DIR");
    ...
    fn test_get_socket_dir_home_fallback() {
        ...
        assert!(result.to_string_lossy().ends_with(".scrapeless-scraping-browser"));
        assert!(
